        ...
        if let Ok(choice) = msg.to_str() {
            match choice.trim() {
                    ...
                    if let Some(Ok(username_msg)) = ws_rx.next().await {
                        if let Ok(username) = username_msg.to_str() {
                            ...
                            match db.login_player(&username).await {
                                Ok(Some(_id)) => {
                                    tx.send(Message::text(format!("Welcome back, {}!", username))).unwrap();
                                    lobby.add_player(username.clone(), tx.clone()).await;

                                }
                                _ => { tx.send(Message::text("Username not found. Try again.")).unwrap(); }
                            }
                        }
                    }
                }
                "2" => { 
                    ...
                    /* Similar implementation as login */ 
                }
                "3" => { tx.send(Message::text("Goodbye!")).unwrap(); return; }
                _ => { tx.send(Message::text("Invalid option.")).unwrap(); }
            }
        }
    }
